...
- 운동, 건강, 피트니스와 전혀 무관한 질문(예: 정치, 날씨, 일반 상식 등)에만 "운동 관련 질문만 답변할 수 있어요 💪"라고 해줘.`;
    ...
    // Prevent array abuse & extract only the tail
    const recentMessages = Array.isArray(messages) ? messages.slice(-MAX_MESSAGES) : [];

    const contents = recentMessages
      .filter(m => m && typeof m.content === 'string')
      .map((m: { role: string; content: string }) => ({
        // Strictly lock roles to allowed values
        role: (m.role === "assistant" || m.role === "model") ? "model" : "user",
        // Prevent extremely long prompt attacks
        parts: [{ text: m.content.substring(0, MAX_CONTENT_LENGTH) }],
      }));
