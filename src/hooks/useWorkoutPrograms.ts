    ...
    try {
      ...
      if (exercises.length > 0) {
        const exercisesToInsert = exercises.map((ex, index) => ({
          ...
          target_sets: parseInt(String(ex.targetSets || 0).replace(/[^0-9]/g, "")) || 0,
          target_reps: parseInt(String(ex.targetReps || 0).replace(/[^0-9]/g, "")) || 0,
          target_weight: parseFloat(String(ex.targetWeight || 0).replace(/[^0-9.]/g, "")) || 0,
          target_distance: ex.targetDistance ? parseFloat(String(ex.targetDistance).replace(/[^0-9.]/g, "")) : null,
          target_time: ex.targetTime ? parseInt(String(ex.targetTime).replace(/[^0-9]/g, "")) : null,
      ...
      toast({
        title: '오류',
        description: `프로그램 생성에 실패했습니다: ${error?.message || JSON.stringify(error)}`,
        variant: 'destructive',
      });
    }
  };
